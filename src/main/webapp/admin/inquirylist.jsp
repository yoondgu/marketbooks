					...
					<div class="page_aticle">
						...
						<div class="page_section">
							<div class="head_aticle">
								<h2 class="tit">1:1문의 관리<span class="tit_sub">모든 1:1문의를 관리할 수 있는 페이지 입니다.</span></h2>
							</div>
   <%
   		// 페이징처리
   		...
   		int totalRows = inquiryDao.getTotalRows();
   		
   		Pagination pagination = new Pagination(totalRows, currentPage);
   		List<InquiryDto> inquiries = inquiryDao.getAllInquiries(pagination.getBeginIndex(), pagination.getEndIndex());
   %>   
	...
	-->
      <nav>
				<ul class="pagination justify-content-center">
					<li class="page-item">
						<a class="page-link <%=pagination.getCurrentPage() == 1 ? "disabled" : "" %>" href="inquirylist.jsp?page=<%=pagination.getCurrentPage() -1%>">이전</a>
					</li>
				<%
					for (int num = pagination.getBeginPage(); num <= pagination.getEndPage(); num++) {
				%>
					<li class="page-item <%=pagination.getCurrentPage() == num ? "active" : "" %>">
						<a class="page-link" href="inquirylist.jsp?page=<%=num %>"><%=num %></a>
					</li>
				<%
					}
				%>
					<li class="page-item">	
						<a class="page-link <%=pagination.getCurrentPage() == pagination.getTotalPages() ? "disabled" : "" %>" href="inquirylist.jsp?page=<%=pagination.getCurrentPage() +1%>">다음</a>
					</li>
				</ul>
	      </nav>
   </div>
...
<div class="page_section">
				<div class="head_aticle">
					<h2 class="tit">상품문의 관리<span class="tit_sub">모든 1:1문의를 관리할 수 있는 페이지 입니다.</span></h2>
				</div>
